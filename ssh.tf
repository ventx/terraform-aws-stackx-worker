...
resource "aws_key_pair" "ssh" {
  ...
  key_name   = substr(lower("${var.cluster_name}${var.static_unique_id != "" ? "-" : ""}${var.static_unique_id != "" ? var.static_unique_id : ""}"), 0, 254)
  public_key = tls_private_key.ssh.0.public_key_openssh
}

resource "aws_security_group" "ssh" {
  count = length(var.ssh_security_group_ids) == 0 ? 1 : 0

  # Up to 255 characters in length. Cannot start with sg- .
  # a-z, A-Z, 0-9, spaces, and ._-:/()#,@[]+=&;{}!$*
  name        = substr(lower("ssh-${var.cluster_name}${var.static_unique_id != "" ? "-" : ""}${var.static_unique_id != "" ? var.static_unique_id : ""}"), 0, 254)
...
resource "aws_security_group_rule" "current_ipv4" {
  ...
  to_port           = 22
  protocol          = "tcp"
  cidr_blocks       = ["${chomp(data.http.current_ipv4.0.body)}/32"]
  type              = "ingress"
...
#  name = substr(lower("ssh-public-${var.cluster_name}${var.static_unique_id != "" ? "-" : ""}${var.static_unique_id != "" ? var.static_unique_id : ""}"), 0, 511)
#
#  tags = local.tags
#}
#
...
#  # Secret names must use Unicode characters.
#  # Secret names contain 1-512 characters.
#  name = substr(lower("ssh-private-${var.cluster_name}${var.static_unique_id != "" ? "-" : ""}${var.static_unique_id != "" ? var.static_unique_id : ""}"), 0, 511)
#
